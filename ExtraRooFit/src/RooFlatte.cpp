...
Double_t RooFlatte::evaluate() const
{
  ...
  std::complex<double> gamma0 = (qSq0 > 0) ? std::complex<double>(g0*sqrt(qSq0),0) :
    std::complex<double>(0, g0*sqrt(-qSq0));

  std::complex<double> gamma1 = (qSq1 > 0) ? std::complex<double>(g1*sqrt(qSq1),0) :
    std::complex<double>(0, g1*sqrt(-qSq1));
  ...
  //std::complex<double> T(mean*sqrt(g0*g1),0);
  std::complex<double> T(1,0);
  T = T / denom;

  return std::pow(std::abs(T),2); // Amplitude (arbitrary scale)
