...
for mode in ${modes[@]}; do
  cutapplier ${mode}_mvaVars.root DecayTree "${phiMcut}&&${phikstveto}&&${LbphiKpveto}&&${Lcphipveto}&&${DtoKaonsveto}&&${Dtophipiveto}" ${mode}_mvaVars_vetoes.root 2>&1| tee log_${mode}.tmp &
done
wait
for mode in ${modes[@]}; do
